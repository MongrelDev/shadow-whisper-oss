...
export function CleanupDiffNudge({
  ...
  return (
    <motion.div
      ...
      role="status"
      aria-live="polite"
      className="flex h-10 items-center gap-3 rounded-full border border-border/60 bg-card pl-[13px] pr-[6px] shadow-[0_10px_30px_-10px_rgba(0,0,0,0.35)]"
    >
      ...
      {hasShortcut ? (
        ...
        <button
          type="button"
          onClick={handleClick}
          className="inline-flex items-center gap-[5px] rounded-md border border-border/60 bg-muted/50 px-2 py-[3px] text-[12.5px] tracking-[-0.005em] text-foreground transition-colors hover:bg-muted"
        >
...
function ShortcutHint({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="inline-flex items-center gap-[5px] rounded-md border border-border/60 bg-muted/50 px-2 py-[3px] text-foreground transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary/60"
    >
      <AlignLeft className="size-[10px] opacity-70" strokeWidth={2.2} aria-hidden />
