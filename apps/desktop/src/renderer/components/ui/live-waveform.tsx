...
export const LiveWaveform = ({
  ...
  useEffect(() => {
    ...
    if (mode === "static") {
      staticBarsRef.current = values;
      lastActiveDataRef.current = values;
    } else {
      historyRef.current = values.slice(-historySize);
      lastActiveDataRef.current = values.slice(-historySize);
    }
    needsRedrawRef.current = true;
  }, [values, mode, historySize]);
  ...
  useEffect(() => {
    ...
    const animate = (currentTime: number) => {
      ...
      if (!values && active && currentTime - lastUpdateRef.current > updateRate) {
